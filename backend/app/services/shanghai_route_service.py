...
GRAPH_NODE_SNAP_RADIUS_DEGREES = 0.03
GRAPH_NODE_CANDIDATE_LIMIT = 12
MAX_ROUTE_LENGTH_RATIO = 4.0

_dataset_lock = threading.Lock()
_dataset_cache: Optional["_ShanghaiRouteDataset"] = None


def is_shanghai_coordinate(lat: float, lon: float) -> bool:
    return (
        SHANGHAI_BOUNDS["south"] <= lat <= SHANGHAI_BOUNDS["north"]
        and SHANGHAI_BOUNDS["west"] <= lon <= SHANGHAI_BOUNDS["east"]
    )
...
class _ShanghaiRouteDataset:
    ...
    def _load_graph(self) -> tuple[dict[str, list[float]], dict[str, list[tuple[str, float]]]]:
        ...
        for feature in data.get("features") or []:
            ...
            route_type = str(props.get("route") or "").strip().lower()
            if route_type not in {"subway", "light_rail", "train"}:
                continue
            ...
            for line in line_sets:
                points = _dedupe_points([[float(latlon[1]), float(latlon[0])] for latlon in line if len(latlon) >= 2])
                if len(points) < 2:
                    continue
                for index in range(len(points) - 1):
    ...
    def build_route(self, lat1: float, lon1: float, lat2: float, lon2: float) -> Optional[dict]:
        ...
        if not start_candidates or not end_candidates:
            logger.info(
                "shanghai_geojson: route failed no_snap_candidates for %.5f,%.5f -> %.5f,%.5f start_candidates=%s end_candidates=%s",
                lat1,
                lon1,
                lat2,
                lon2,
        ...
        if not best_geometry or best_start_point is None or best_end_point is None:
            logger.info(
                "shanghai_geojson: route failed no_connected_path for %.5f,%.5f -> %.5f,%.5f start_candidates=%s end_candidates=%s",
                lat1,
                lon1,
                lat2,
                lon2,
        ...
        if direct_length > 0 and route_length > direct_length * MAX_ROUTE_LENGTH_RATIO:
            logger.info(
                "shanghai_geojson: route failed length_ratio for %.5f,%.5f -> %.5f,%.5f route=%.6f direct=%.6f max_ratio=%.2f",
                lat1,
                lon1,
                lat2,
                lon2,
        ...
        logger.info(
            "shanghai_geojson: route accepted for %.5f,%.5f -> %.5f,%.5f points=%s",
            lat1,
            lon1,
            lat2,
            lon2,
...
def _get_dataset() -> Optional[_ShanghaiRouteDataset]:
    global _dataset_cache

    configured_path = Path(settings.SHANGHAI_RAIL_GEOJSON_PATH.strip()) if settings.SHANGHAI_RAIL_GEOJSON_PATH.strip() else None
    if not configured_path:
        return None
    if not configured_path.is_absolute():
        configured_path = (Path(__file__).resolve().parents[3] / configured_path).resolve()
    if not configured_path.exists():
        logger.warning("shanghai_geojson: configured file not found at %s", configured_path)
        return None

    with _dataset_lock:
        if _dataset_cache and _dataset_cache.path == configured_path:
            return _dataset_cache
        _dataset_cache = _ShanghaiRouteDataset(configured_path)
        return _dataset_cache


def build_route_from_shanghai_geojson(lat1: float, lon1: float, lat2: float, lon2: float) -> Optional[dict]:
    if not (is_shanghai_coordinate(lat1, lon1) and is_shanghai_coordinate(lat2, lon2)):
        return None
