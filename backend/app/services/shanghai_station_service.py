...
from .shanghai_route_service import is_shanghai_coordinate
...
def _preferred_station_name(properties: dict) -> str:
    return (
        ...
        or str(properties.get("alt_name:en") or "").strip()
        or str(properties.get("name") or "").strip()
        or "Shanghai Metro Station"
    )
...
def _resolve_dataset_path() -> Optional[Path]:
    configured_path = settings.SHANGHAI_STATION_GEOJSON_PATH.strip()
    ...
    if not path.exists():
        logger.warning("shanghai_station_geojson: configured file not found at %s", path)
        return None
    return path
...
def _load_dataset() -> list[dict]:
    ...
    with _dataset_lock:
        ...
        for feature in data.get("features") or []:
            ...
            if str(properties.get("station") or "").strip().lower() != "subway":
                continue
            ...
            stations.append(
                {
                    ...
                    "city": "Shanghai",
                    "country": "China",
                    "railway_type": str(properties.get("station") or properties.get("railway") or "subway"),
                    "tags": properties,
        ...
        logger.info("shanghai_station_geojson: loaded %s station candidates", len(stations))
        return _dataset_cache


def lookup_nearest_station_from_shanghai_geojson(lat: float, lon: float) -> Optional[dict]:
    if not is_shanghai_coordinate(lat, lon):
        return None
    ...
    return {
        ...
        "source": "shanghai_station_geojson",
        "tags": best_station["tags"],
        "osm_id": best_station["osm_id"],
    }


def search_stations_from_shanghai_geojson(query: str, limit: int = 10) -> list[dict]:
    ...
    for station in stations:
        ...
        ranked.append(
            (
                best_score,
                {
                    "id": station.get("osm_id") or f"shanghai-{station['latitude']:.6f}-{station['longitude']:.6f}",
                    "place_name": station["name"],
                    "city": station["city"],
                    "country": station["country"],
                    "latitude": station["latitude"],
                    "longitude": station["longitude"],
                    "subtitle": f"{station['city']}, {station['country']}",
                    "source": "shanghai_station_geojson",
                },
            )
        )
